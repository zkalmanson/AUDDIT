...
pxlSize = 0.1032; % San Miguel Lab: 0.1032 µm (rotenone/cold shock); Meyer Lab: 0.38 µm (6-OHDA)
...
% for fldr = 5 % Use this if you want to look at only one folder
...
%     for img_num = 2 % Use this line if you only want to look at a specific image
        ...
        totBreakPrp = totBreak./size(imLocCont,1);
        ...
        f = gcf;
        exportgraphics(f,sprintf('coldFeatures.pdf'),'Resolution',100,'Append',true)
        ...
        title(sprintf('Folder: %d Image: %d',fldr,img_num),'FontSize',26)
        f = gcf;
        exportgraphics(f,sprintf('coldBreaks.pdf'),'Resolution',100,'Append',true)
        ...
        title(sprintf('Folder: %d Image: %d',fldr,img_num),'FontSize',26)
        f = gcf;
        exportgraphics(f,sprintf('coldTracks.pdf'),'Resolution',100,'Append',true) 
...
% Save data as .mat file for later analysis (un-comment what is wanted)

% Feature results
save('coldFeatureTable.mat','MasterFeatureTable','blebsNoneTable');

% Images
save('coldCroppedImages.mat','croppedImages'); 
